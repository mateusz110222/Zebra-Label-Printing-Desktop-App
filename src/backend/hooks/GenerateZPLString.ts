...
import { fillZplTemplate, calculateSerial } from './LabelProcessor'
...
export default async function (
  ...
  try {
    ...
    if (!rawTemplate) throw new Error(`Szablon ${formatName} jest pusty.`)
  } catch (err: any) {
  ...
  try {
    ...
    const errorMsg = (error instanceof Error ? error.message : String(error)) || 'Nieznany błąd'
